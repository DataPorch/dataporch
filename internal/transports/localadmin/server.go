...
func (s *Server) Run(ctx context.Context) error {
	...
	listener, err := net.Listen("unix", s.path) //nolint:noctx // Shutdown controls the listener lifetime.
	if err != nil {
		return fmt.Errorf("listening on admin socket: %w", err)
	}

	if err := os.Chmod(s.path, 0o660); err != nil { //nolint:gosec // Group access is required for local clients.
		_ = listener.Close()
		_ = os.Remove(s.path)
...
func prepareSocket(path string) error {
	...
	connection, err := net.DialTimeout( //nolint:noctx // The socket probe is bounded by a short timeout.
		"unix",
		path,
